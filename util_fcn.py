...
class DefineParameters:
    def __init__(self, batch_train=0, epochs=1, batch_test=1, axis_select=0, num_subject=None , mri_type = 3):
        ...
        file_to_feed = FilesNamesAndTypes(self.mri_type, self.num_subject)
        file_to_feed.select_files_name_type()

        self.files = file_to_feed.get_files()
...
def labels_func():
    targets = np.zeros((int(np.ceil(900/1.51069)),2))
    for i in range(15):
        targets[40*i:40*i+30] = [1,0] #Rest
        targets[(40*(i+1))-10:40*(i+1)] = [0,1] #Dist
    return targets 

def labels_Naive_CPH(name):
    targets = np.zeros((int(np.ceil(900/1.51069)),2))
    naive = ["sub-124","sub-157","sub-158","sub-159","sub-160","sub-161"]
    CPH = ["sub-077","sub-079","sub-081","sub-082","sub-083","sub-084"]
